...
def raycast_scene_skip_backfaces(depsgraph, scene, origin, direction,
    ...
    return raycast_scene_skip_hidden_and_backfaces(
        depsgraph, scene, origin, direction, max_iterations, None
    )


def raycast_scene_skip_hidden_and_backfaces(depsgraph, scene, origin, direction,
                                            max_iterations, is_hidden_face):
    """Raycast through the scene, skipping hidden faces and culled backfaces.

    is_hidden_face, when provided, is called as (object, face_index) and should
    return True for edit-mode-hidden faces.
    """
    ...
    for _ in range(max_iterations):
        ...
        if is_hidden_face is not None and is_hidden_face(obj, face_index):
